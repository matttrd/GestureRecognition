...
class SAXModel(object):
    	...
    	def paa_window(self, window_signal,augmented = False):
         """
         piecewise aggregate approximation: one sliding window signal to a word
         """
         if type(window_signal) == np.float64:
             raise "s is a number"
         (s,mean,sd) = self.whiten(window_signal)
         binsize = Fraction(len(s), self.nbins)
         xs = map(lambda ss: np.sum(ss) / float(binsize), self.binpack(s))
         if augmented:
             return (xs,mean,sd,len(window_signal))
         else:
             return xs
               
    	def symbolize_window(self, window_signal,augmented = False,brd = 0):
         """
         Symbolize one sliding window signal to a word
         """
         (xs,mean,var,lw) = self.paa_window(window_signal,augmented = True)
         if augmented:
             if brd:
                return ("".join(self.symbolize(xs))[brd:-brd],mean,var, lw) 
             else:
                 return ("".join(self.symbolize(xs)),mean,var, lw)
         else:
             if brd:
                return "".join(self.symbolize(xs))[brd:-brd] 
             else:
    	...
    	def word_templates_dist(self, word1, template_list):
          min_dist = min([self.symbol_distance(word1, word) for word in template_list])
          return min_dist
